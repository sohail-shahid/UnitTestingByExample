...
final class MockPasswordChanger: PasswordChangerProtocol {
    ...
    private func changeWasCalledOnce( file: StaticString = #file, line: UInt = #line) -> Bool {
        refactoringVerifyMethodCalledOnce( methodName: changeMethodName, callCount: changeCallCount, describeArguments: changeMethodArguments, file: file, line: line)
    }
    ...
    private var changeMethodName: String { "change(securityToken:oldPassword:newPassword:onSuccess:onFailure:)"
    }
    
    private var changeMethodArguments: String { "oldPasswords: \(changeArgsOldPassword), " +
